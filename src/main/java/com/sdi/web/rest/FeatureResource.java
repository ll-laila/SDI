...
public class FeatureResource {
        ...
        Optional<Feature> result = featureRepository
            ...
            .map(existingFeature -> {
                ...
                if (feature.getNotes() != null) {
                    existingFeature.setNotes(feature.getNotes());
                }
                if (feature.getCreaDate() != null) {
                    existingFeature.setCreaDate(feature.getCreaDate());
                }
     ...
     */
    @GetMapping("")
    public List<Feature> getAllFeatures(@RequestParam(name = "eagerload", required = false, defaultValue = "true") boolean eagerload) {
        LOG.debug("REST request to get all Features");
        if (eagerload) {
            return featureRepository.findAllWithEagerRelationships();
        } else {
    ...
    public ResponseEntity<Feature> getFeature(@PathVariable("id") Long id) {
        ...
        Optional<Feature> feature = featureRepository.findOneWithEagerRelationships(id);
