...
public interface ModuleDeployementRepository extends JpaRepository<ModuleDeployement, Long> {
    ...
    @Query(
        value = "select moduleDeployement from ModuleDeployement moduleDeployement left join fetch moduleDeployement.module left join fetch moduleDeployement.productDeployement left join fetch moduleDeployement.custoLevel",
    ...
    @Query(
        "select moduleDeployement from ModuleDeployement moduleDeployement left join fetch moduleDeployement.module left join fetch moduleDeployement.productDeployement left join fetch moduleDeployement.custoLevel"
    )
    List<ModuleDeployement> findAllWithToOneRelationships();

    @Query(
        "select moduleDeployement from ModuleDeployement moduleDeployement left join fetch moduleDeployement.module left join fetch moduleDeployement.productDeployement left join fetch moduleDeployement.custoLevel where moduleDeployement.id =:id"
    )
