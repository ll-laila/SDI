...
public interface FeatureDeployementRepository extends JpaRepository<FeatureDeployement, Long> {
    ...
    @Query(
        value = "select featureDeployement from FeatureDeployement featureDeployement left join fetch featureDeployement.feature left join fetch featureDeployement.custoLevel",
    ...
    @Query(
        "select featureDeployement from FeatureDeployement featureDeployement left join fetch featureDeployement.feature left join fetch featureDeployement.custoLevel"
    )
    List<FeatureDeployement> findAllWithToOneRelationships();

    @Query(
        "select featureDeployement from FeatureDeployement featureDeployement left join fetch featureDeployement.feature left join fetch featureDeployement.custoLevel where featureDeployement.id =:id"
    )
