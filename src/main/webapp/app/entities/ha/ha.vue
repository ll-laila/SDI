<template>
  <div>
    <h2 id="page-heading" data-cy="HAHeading">
      <span v-text="t$('sdiApp.hA.home.title')" id="ha-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" @click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('sdiApp.hA.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'HACreate' }" custom v-slot="{ navigate }">
          <button @click="navigate" id="jh-create-entity" data-cy="entityCreateButton" class="btn btn-primary jh-create-entity create-ha">
            ...
            <span v-text="t$('sdiApp.hA.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && hAS && hAS.length === 0">
      <span v-text="t$('sdiApp.hA.home.notFound')"></span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="hAS">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="t$('sdiApp.hA.name')"></span></th>
            <th scope="row"><span v-text="t$('sdiApp.hA.creaDate')"></span></th>
            <th scope="row"><span v-text="t$('sdiApp.hA.updateDate')"></span></th>
            <th scope="row"><span v-text="t$('sdiApp.hA.notes')"></span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span id="sdiApp.hA.delete.question" data-cy="hADeleteDialogHeading" v-text="t$('entity.delete.title')"></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-hA-heading" v-text="t$('sdiApp.hA.delete.question', { id: removeId })"></p>
      </div>
