<template>
  <div>
    <h2 id="page-heading" data-cy="ClientEventTypeHeading">
      <span v-text="t$('sdiApp.clientEventType.home.title')" id="client-event-type-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" @click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('sdiApp.clientEventType.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'ClientEventTypeCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="t$('sdiApp.clientEventType.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && clientEventTypes && clientEventTypes.length === 0">
      <span v-text="t$('sdiApp.clientEventType.home.notFound')"></span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="clientEventTypes">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="t$('sdiApp.clientEventType.type')"></span></th>
            <th scope="row"><span v-text="t$('sdiApp.clientEventType.description')"></span></th>
            <th scope="row"><span v-text="t$('sdiApp.clientEventType.creaDate')"></span></th>
            <th scope="row"><span v-text="t$('sdiApp.clientEventType.updateDate')"></span></th>
            <th scope="row"><span v-text="t$('sdiApp.clientEventType.notes')"></span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="clientEventType in clientEventTypes" :key="clientEventType.id" data-cy="entityTable">
            ...
            <td>{{ clientEventType.creaDate }}</td>
            <td>{{ clientEventType.updateDate }}</td>
            <td>{{ clientEventType.notes }}</td>
            <td class="text-right">
              <div class="btn-group">
                <router-link
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span
          id="sdiApp.clientEventType.delete.question"
          data-cy="clientEventTypeDeleteDialogHeading"
          v-text="t$('entity.delete.title')"
        ></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-clientEventType-heading" v-text="t$('sdiApp.clientEventType.delete.question', { id: removeId })"></p>
      </div>
