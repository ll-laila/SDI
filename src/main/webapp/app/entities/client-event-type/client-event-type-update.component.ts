      ...
      if (this.clientEventType.id) {
        this.clientEventTypeService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showInfo(this.t$('sdiApp.clientEventType.updated', { param: param.id }));
          })
          .catch(error => {
            this.isSaving = false;
        ...
        this.clientEventTypeService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showSuccess(this.t$('sdiApp.clientEventType.created', { param: param.id }).toString());
          })
          .catch(error => {
            this.isSaving = false;
            this.alertService.showHttpError(error.response);
