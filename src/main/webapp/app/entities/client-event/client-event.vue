<template>
  <div>
    <h2 id="page-heading" data-cy="ClientEventHeading">
      <span v-text="t$('sdiApp.clientEvent.home.title')" id="client-event-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" @click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('sdiApp.clientEvent.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'ClientEventCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="t$('sdiApp.clientEvent.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && clientEvents && clientEvents.length === 0">
      <span v-text="t$('sdiApp.clientEvent.home.notFound')"></span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="clientEvents">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="t$('sdiApp.clientEvent.event')"></span></th>
            <th scope="row"><span v-text="t$('sdiApp.clientEvent.description')"></span></th>
            <th scope="row"><span v-text="t$('sdiApp.clientEvent.creaDate')"></span></th>
            <th scope="row"><span v-text="t$('sdiApp.clientEvent.updateDate')"></span></th>
            <th scope="row"><span v-text="t$('sdiApp.clientEvent.notes')"></span></th>
            <th scope="row"><span v-text="t$('sdiApp.clientEvent.client')"></span></th>
            <th scope="row"><span v-text="t$('sdiApp.clientEvent.clientEventType')"></span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="clientEvent in clientEvents" :key="clientEvent.id" data-cy="entityTable">
            ...
            <td>{{ clientEvent.creaDate }}</td>
            <td>{{ clientEvent.updateDate }}</td>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span id="sdiApp.clientEvent.delete.question" data-cy="clientEventDeleteDialogHeading" v-text="t$('entity.delete.title')"></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-clientEvent-heading" v-text="t$('sdiApp.clientEvent.delete.question', { id: removeId })"></p>
      </div>
