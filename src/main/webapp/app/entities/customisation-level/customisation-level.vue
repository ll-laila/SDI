<template>
  <div>
    <h2 id="page-heading" data-cy="CustomisationLevelHeading">
      <span v-text="t$('sdiApp.customisationLevel.home.title')" id="customisation-level-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" @click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('sdiApp.customisationLevel.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'CustomisationLevelCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="t$('sdiApp.customisationLevel.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && customisationLevels && customisationLevels.length === 0">
      <span v-text="t$('sdiApp.customisationLevel.home.notFound')"></span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="customisationLevels">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="t$('sdiApp.customisationLevel.level')"></span></th>
            <th scope="row"><span v-text="t$('sdiApp.customisationLevel.creaDate')"></span></th>
            <th scope="row"><span v-text="t$('sdiApp.customisationLevel.updateDate')"></span></th>
            <th scope="row"><span v-text="t$('sdiApp.customisationLevel.notes')"></span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="customisationLevel in customisationLevels" :key="customisationLevel.id" data-cy="entityTable">
            ...
            <td>{{ customisationLevel.creaDate }}</td>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span
          id="sdiApp.customisationLevel.delete.question"
          data-cy="customisationLevelDeleteDialogHeading"
          v-text="t$('entity.delete.title')"
        ></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-customisationLevel-heading" v-text="t$('sdiApp.customisationLevel.delete.question', { id: removeId })"></p>
      </div>
