      ...
      if (this.customisationLevel.id) {
        this.customisationLevelService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showInfo(this.t$('sdiApp.customisationLevel.updated', { param: param.id }));
          })
          .catch(error => {
            this.isSaving = false;
        ...
        this.customisationLevelService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showSuccess(this.t$('sdiApp.customisationLevel.created', { param: param.id }).toString());
          })
          .catch(error => {
            this.isSaving = false;
            this.alertService.showHttpError(error.response);
