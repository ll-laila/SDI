<template>
  <div>
    <h2 id="page-heading" data-cy="DeployementTypeHeading">
      <span v-text="t$('sdiApp.deployementType.home.title')" id="deployement-type-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" @click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('sdiApp.deployementType.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'DeployementTypeCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="t$('sdiApp.deployementType.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && deployementTypes && deployementTypes.length === 0">
      <span v-text="t$('sdiApp.deployementType.home.notFound')"></span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="deployementTypes">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="t$('sdiApp.deployementType.type')"></span></th>
            <th scope="row"><span v-text="t$('sdiApp.deployementType.creaDate')"></span></th>
            <th scope="row"><span v-text="t$('sdiApp.deployementType.updateDate')"></span></th>
            <th scope="row"><span v-text="t$('sdiApp.deployementType.notes')"></span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="deployementType in deployementTypes" :key="deployementType.id" data-cy="entityTable">
            ...
            <td>{{ deployementType.creaDate }}</td>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span
          id="sdiApp.deployementType.delete.question"
          data-cy="deployementTypeDeleteDialogHeading"
          v-text="t$('entity.delete.title')"
        ></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-deployementType-heading" v-text="t$('sdiApp.deployementType.delete.question', { id: removeId })"></p>
      </div>
